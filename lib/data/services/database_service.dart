...
class DatabaseService {
  ...
  Future<void> _seedSymptoms(Database db) async {
    ...
    for (final s in symptoms) {
      await db.insert('symptom', s);
    }
  }

  Future<void> _seedBadges(Database db) async {
    final badges = [
      {'title': '1 Minggu Disiplin', 'description': 'Minum obat 7 hari berturut-turut', 'required_streak': 7},
      {'title': '2 Minggu Disiplin', 'description': 'Minum obat 14 hari berturut-turut', 'required_streak': 14},
      {'title': '1 Bulan Disiplin', 'description': 'Minum obat 30 hari berturut-turut', 'required_streak': 30},
    ];
  ...
  Future<Map<String, dynamic>?> getUserByEmail(String email) async {
    final db = await database;
    final results = await db.query('user', where: 'email = ?', whereArgs: [email]);
    return results.isNotEmpty ? results.first : null;
  }
  ...
  Future<void> updateTreatmentPlan(int id, Map<String, dynamic> data) async {
    final db = await database;
    await db.update('treatment_plan', data, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> getMedicines(int userId) async {
    final db = await database;
    return db.query('medicine', where: 'user_id = ?', whereArgs: [userId]);
  }
  ...
  Future<Map<String, dynamic>?> getTodayMonitoring(int medicineId, String date) async {
    final db = await database;
    final results = await db.query(
      'monitoring',
  ...
  Future<void> updateMonitoring(int id, Map<String, dynamic> data) async {
    final db = await database;
    await db.update('monitoring', data, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> getMonitoringDates(int userId, String status) async {
    final db = await database;
    return db.rawQuery('''
      SELECT m.date FROM monitoring m
      JOIN medicine med ON m.medicine_id = med.id
  ...
  Future<List<Map<String, dynamic>>> getSymptomsForMonitoring(int monitoringId) async {
    final db = await database;
    return db.rawQuery('''
  ...
  Future<void> insertMonitoringSymptom(int monitoringId, int symptomId) async {
    final db = await database;
  ...
  Future<Map<String, dynamic>?> getMissedDoseByDate(int medicineId, String date) async {
    final db = await database;
    final results = await db.query(
      'missed_dose',
  ...
  Future<void> insertMissedDose(Map<String, dynamic> data) async {
    final db = await database;
    await db.insert('missed_dose', data);
  }
  ...
  Future<List<Map<String, dynamic>>> getUserBadges(int userId) async {
    final db = await database;
    return db.rawQuery('''
      SELECT b.*, ub.earned_at FROM badge b
  ...
  Future<int> calculateStreak(int userId) async {
    ...
    final results = await db.rawQuery('''
      SELECT DISTINCT m.date FROM monitoring m
      JOIN medicine med ON m.medicine_id = med.id
